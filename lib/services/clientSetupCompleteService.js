...
// Try multiple times with increasing delay
    ...
    parseFAQs(faqText) {
        ...
        for (const line of lines) {
            ...
            // Match "Question:", "Questions:", "Q:", or "Q." (case insensitive)
            if (lowerLine.startsWith('question:') || lowerLine.startsWith('questions:') ||
                lowerLine.startsWith('q:') || lowerLine.match(/^q\d*[:.]/)) {
                ...
                // Start new question - find where the actual question starts after the prefix
                const questionMatch = trimmedLine.match(/^(questions?|q\d*)[:\.]?\s*(.+)/i);
                currentQuestion = questionMatch ? questionMatch[2].trim() : trimmedLine.substring(9).trim();
                currentAnswer = null;
            } else if (lowerLine.startsWith('answer:') || lowerLine.startsWith('answers:') ||
                       lowerLine.startsWith('a:') || lowerLine.match(/^a\d*[:.]/)) {
                // Match "Answer:", "Answers:", "A:", or "A." (case insensitive)
                const answerMatch = trimmedLine.match(/^(answers?|a\d*)[:\.]?\s*(.+)/i);
                currentAnswer = answerMatch ? answerMatch[2].trim() : trimmedLine.substring(7).trim();
